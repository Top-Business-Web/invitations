                ...
                <div class="page slidePage" id="div1">
                    ...
                    <div class="">
                        <input name="image" id="image" type="file" class="dropify"
                               data-default-file="{{ asset($invite->image) }}">
                    </div>
                    <div class="row g-3">

                        <div class="col-md-6 col-12">
                            <label class="form-label">تاريخ المناسبة</label>
                            <input type="text" name="date" value="{{ $invite->date }}" class="form-control input-field"
                                   id="datepicker">
                        </div>
                        <div class="col-md-6 col-12">
                            <label class="form-label">اسم المناسبة</label>
                            <input type="text" name="title" value="{{ $invite->title }}"
                                   class="form-control input-field" id="title">
                        </div>
                        ...
                        <div class="col-md-12 col-12">
                            ...
                            <input name="address" value="{{ $invite->address }}" id="searchMapInput" type="text"
                                   class="form-control mapControls input-field">
                            <div id="map"></div>

                            <div style="display: none" id="geoData">
                                <input style="border: 1px solid #ccc" value="{{ $invite->address }}" name="address"
                                       type="text" id="location-snap">
                                <input style="border: 1px solid #ccc" value="{{ $invite->longitude }}" name="longitude"
                                       type="text" id="lng-span">
                                <input style="border: 1px solid #ccc" value="{{ $invite->latitude }}" name="latitude"
                                       type="text" id="lat-span">
                            </div>
                        ...
                        <div class="col-12 mb-2">
                            ...
                            <input class="form-check-input" name="has_qrcode"
                                   {{ ($invite->has_qrcode == 1) ? 'checked' : '' }} value="1" type="checkbox"
                        ...
                        <div class="col-lg-8 col-12 d-flex mt-4 justify$invite->invitees-content-end">
                            <a href="#" class="text-decoration-none btn-login" id="addDraftInvite"
                               style="background-color: #C7C7C7;"> حفظ فى المسودات</a>
                            <button type="button" id="step1BtnEdit" data-id="{{$invite->id}}"
                                    class="next-btn main-btn1 step1Btn">حفظ ومتابعة
                            </button>
                        </div>
                    </div>
                </div>
...
{{--                                    @foreach($invite->invitees as $invitee)--}}
{{--                                        @if($contact->phone == $invitee->phone)--}}
{{--                                            @php--}}
{{--                                                $isSelected = true;--}}
{{--                                            @endphp--}}
{{--                                        @endif--}}
{{--                                    @endforeach--}}
{{--                                    <td scope="row">--}}
{{--                                        <input type="checkbox" class="custom-control-input services" name="service_id[]"--}}
                            ...
                            @foreach ($contacts as $index => $contact)
                                <tr>
                                    ...
                                    <td scope="row">
                                        ...
                                        @endforeach
                                        <input type="checkbox" class="custom-control-input services" name="service_id[]"
                                               value="{{ $contact->id }}" {{ $isChecked ? 'checked' : '' }}>
                                    </td>
                                    <td>{{ $contact->name }}</td>
                                    <td>{{ $contact->email ?? '-' }}</td>
                                    <td>
